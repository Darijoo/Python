...
def schrijven_naar_html(logs_list):
    ...
    with open("web/template.html", "r") as input_bestand:
    ...
    logs_list_kopie.sort(
        key=lambda date: datetime.strptime(date[0], "%Y-%m-%d %X"), reverse=True
    )
    for event in logs_list_kopie:
        if event[5][0] == None:
            ...
            rapportering = (
                f"Min: {event[5][1]}ms | Avg: {event[5][2]}ms |  Max: {event[5][3]}ms"
            )
        if succes == 100:
    ...
    with open("web/logging.html", "w") as output_bestand:
